...
import org.source.util.XMLWriter;
...
public class Connection {
    ...
    public void deliver(Packet packet) {
        ...
        if (!isClosed()) {
            ...
            try {
                XMLWriter xmlSerializer = new XMLWriter(new IoBufferWriter(buffer, (CharsetEncoder) encoder.get()),new OutputFormat());
        ...
        if (!isClosed()) {
            ...
            try {
                ...
                if (asynchronous) {
                    ...
                    boolean ok = ioSession.write(buffer).awaitUninterruptibly(Config.getInt("connection.ack.timeout", 2000));
                    if (!ok) {
